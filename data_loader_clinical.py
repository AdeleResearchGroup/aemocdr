...
import import_ipynb
import utils  # Va chercher le fichier utils.ipynb
...
class RawDataLoader:
    ...
    @staticmethod
    def load_clinical_data(file_path, index_col='Cell_Line'):

        """
        Charge le fichier TSV des données cliniques et effectue un prétraitement des données.
    
        Parameters:
        - file_path (str): Chemin vers le fichier TSV (matched_cell_lines_annotations.tsv).
        - index_col (str): Nom de la colonne à utiliser comme index (ex: nom de la lignée cellulaire).
    
        Returns:
        - pd.DataFrame: DataFrame avec les 11 colonnes cliniques.
        """
        # Charger le fichier TSV
        clinical_df = pd.read_csv(file_path, sep='\t', index_col=index_col)
        
        # Application d'un prétraitement (normalisation et encodage des variables catégorielles)
        # Pour les variables catégorielles: one-hot encoding
        # Pour les variables numériques: normalisation

        # Identify numeric and categoric columns
        numeric_cols = ['Age']
        categorical_cols = [col for col in clinical_df.columns if col not in numeric_cols]
        
        # Replace missing values for numeric coloumn by the median
        clinical_df['Age'].fillna(clinical_df['Age'].median(), inplace=True)

        # Normalisation with StandardScaler()
        scaler = StandardScaler()
        clinical_df['Age'] = scaler.fit_transform(clinical_df[['Age']])
        
        # Pour les colonnes catégorielles, remplacer les valeurs manquantes par "Missing"
        for col in categorical_cols:
            clinical_df[col] = clinical_df[col].fillna("Missing")

        # Encodage one-hot pour les colonnes catégorielles
        clinical_df_encoded = pd.get_dummies(clinical_df, columns=categorical_cols)
        
        clinical_df_encoded = clinical_df_encoded.astype('float32')
