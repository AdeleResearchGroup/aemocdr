...
def weighted_mse_loss(recon, target, alpha=1):
    """
    alpha > 1 => puts more weight on non-zero targets
    alpha = 1 => standard MSE
    """
...
def knn_radius_stats(Z, k: int = 10):
    """
    Z: (n_samples, d) tensor or ndarray of latent vectors.
    Returns: mean_radius (↓ = better), std_radius, cv_radius = std/mean.
    We standardize per feature for a fair comparison across models.
    """
