...
def TriMORDR_pretrained_training(x_cell_train, x_drug_train, y_train, visualize='first', run_id=0):
    """
...
def cv_train(x_cell_train, x_drug_train, y_train, device, k=5, visualize='first', run_id=0):
...
def run(k=10, is_test=False, visualize='first'):
    ...
    for i in range(k):
        ...
        if is_test:
            ...
            model, cell_norms, drug_norms = TriMORDR_pretrained_training(x_cell_train, x_drug_train, y_train, visualize=visualize, run_id=i)
            ...
            # save tensors for CCLE few-shot phase 3
            torch.save(
                {
                    'Xc': x_cell_test_tensor.detach().cpu(),
                    'Xd': x_drug_test_tensor.detach().cpu(),
                    'y' : y_test_tensor.detach().cpu()
                },
                f"ccle_test_tensors_run_{run_id}.pt" 
            )
            ...
            # Train and evaluate the TriMORDR on the split data
            results = cv_train(x_cell_train, x_drug_train, y_train, device, k=2, visualize='first', run_id=i)
    ...
    if is_test and len(all_runs) > 0:
        ...
        Evaluation.plot_mean_roc(fpr_grid, mean_tpr, std_tpr, auc_mean, auc_std, label="TriMOR-DR")
        Evaluation.plot_mean_pr(rec_grid, mean_prec, std_prec, auprc_mean, auprc_std, label="TriMOR-DR")
