    ...
    for fold, (train_data, val_data) in enumerate(cv.split(np.zeros(len(y_np)), y_np, groups)):
        ...
        if visualize == 'always' or (visualize == 'first' and fold == 0):
            with torch.no_grad():
                ...
                z_cell_fold = model.encoder_cell(x_cell_tr_t)
                z_drug_fold = model.encoder_drug(x_drug_tr_t)
                ...
                plot_tsne(z_cell_fold, y_tr_t, "t-SNE - z_cell")
                plot_tsne(z_drug_fold, y_tr_t, "t-SNE - z_drug")
                plot_tsne(torch.cat([z_cell_fold, z_drug_fold], dim=1), y_tr_t, "t-SNE - z_cell + z_drug")
...
def run(k=10, is_test=False, visualize='first'):
    ...
    for i in range(k):
        ...
        if is_test:
            ...
            results = cv_train(x_cell_train, x_drug_train, y_train, device, k=5, run_id=i, visualize='first', split='LDO')
