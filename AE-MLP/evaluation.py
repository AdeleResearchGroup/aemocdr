...
from sklearn.metrics import roc_auc_score, average_precision_score, confusion_matrix, f1_score, precision_score, recall_score, accuracy_score, roc_curve, precision_recall_curve, balanced_accuracy_score
...
class Evaluation:
    ...
    def aggregate_roc(all_runs, n_points=200):
        ...
        Retourne: fpr_grid, mean_tpr, std_tpr, auc_mean, auc_std
        """
        ...
        for y_true, y_score in all_runs:
            ...
            # Interpole TPR sur la grille FPR commune
    ...
    def aggregate_pr(all_runs, n_points=200):
        """
        Agrège les courbes PR en interpolant Precision(recall).
        Retourne: recall_grid, mean_prec, std_prec, auprc_mean, auprc_std
        """
        ...
        for y_true, y_score in all_runs:
            ...
            # sklearn renvoie recall croissant ; on interpole Precision en fct de Recall
            # NB: pour éviter des oscillations, on peut forcer une enveloppe concave si besoin,
            # mais ici l'interp linéaire suffit.
            # On s'assure que rec est strictement croissant pour np.interp
    ...
    @staticmethod
    def evaluate(all_targets, mlp_output, show_plot=False, threshold=0.5):
        """
        ...
        # 1) tensors/array -> numpy 1D (robuste aux deux types)
        ...
        y_true  = y_true.astype(int)     # s'assurer 0/1
        y_score = y_score.astype(float)  # scores/probas
    
        # 2) Binarisation au seuil (par défaut 0.5)
        y_pred = (y_score >= threshold).astype(int)
    
        # 3) Métriques alignées sur la classe positive = 1 (sensitive)
        ...
        # AUC / AUPRC à partir des probabilités
        fpr, tpr, _ = roc_curve(y_true, y_score, pos_label=1)
        ...
        if show_plot:
            ...
            fig, ax = plt.subplots()
            ax.set_ylabel("DeepDRA score (P[class=1|x])")
            ax.set_xticks([1, 2])
