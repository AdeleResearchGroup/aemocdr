...
def test(model, test_loader, show_plot=False, return_raw=False):
    """
    Évalue le modèle sur test_loader et retourne les métriques Evaluation.evaluate.
    - On force return_latent=False pour n'avoir que 3 sorties (decode_cell, decode_drug, mlp_output).
    - On agrège les prédictions/labels sur toutes les batches.
    """
    model.eval()
