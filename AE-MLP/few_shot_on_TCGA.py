...
# ====== Data helpers ======
def make_zero_shot_loader(tcga_cell, tcga_drug, y_np, cell_norms, drug_norms, batch_size=64):
    x_cell = torch.tensor(tcga_cell.values, dtype=torch.float32)
    x_drug = torch.tensor(tcga_drug.values, dtype=torch.float32)
    y_t    = torch.tensor(np.asarray(y_np).reshape(-1, 1), dtype=torch.float32)
    # Use provided train-time norms; guard tiny values as in previous scripts
    thr = 1e-6
    cell_norms = torch.norm(x_cell, dim=0, keepdim=True)
    cell_norms = torch.where(cell_norms < thr, torch.ones_like(cell_norms), cell_norms)
    drug_norms = torch.norm(x_drug, dim=0, keepdim=True)
...
def make_few_shot_loaders(tcga_cell, tcga_drug, y_np, K, cell_norms, drug_norms, seed=42, batch_size=64, use_rus=True):
    full_df = pd.concat([tcga_cell, tcga_drug], axis=1)
    n_cell  = tcga_cell.shape[1]
    ...
    # Tensors + normalization fitted on support
...
def run_one_few_shot(
    tcga_cell, tcga_drug, y_np, K_support,
    ...
    # ---------- Zero-shot ----------
    if K_support == 0:
        val_loader = make_zero_shot_loader(tcga_cell, tcga_drug, y_np, batch_size=batch_size, cell_norms=cell_norms, drug_norms=drug_norms)
        ...
        # Extra zero-shot logging fields
        res["K_post"] = 0
        res["n_pos_post"] = 0
        res["n_neg_post"] = 0
        return res
    ...
    train_loader, val_loader, pos_weight_scalar, info = make_few_shot_loaders(
        tcga_cell, tcga_drug, y_np, K_support, cell_norms, drug_norms, seed=seed, batch_size=batch_size, use_rus=use_rus
    )
    ...
    # Loss & optional class weighting (if RUS is not used)
    ...
    patience, bad = 8, 0  # early stop on AUPRC
...
def few_shot_sweep_TCGA(run_id=0,
    ...
    for key in ['cell_exp', 'drug_desc', 'drug_finger']:
    ...
    # Ensure shape [1, D]
    if cell_norms.ndim == 1: cell_norms = cell_norms.unsqueeze(0)
    if drug_norms.ndim == 1: drug_norms = drug_norms.unsqueeze(0)
