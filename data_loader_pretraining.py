...
class RawDataLoader:
    ...
    @staticmethod
    def get_unique_entities(data_dict):
        """
        Retourne X_cell_full et X_drug_full à partir du data_dict 
        (une ligne par cellule/drogue, toutes modalités concaténées)
        """
        # Concatenate cell modalities
        cell_data_types = sorted([mod for mod in data_dict.keys() if mod.startswith("cell")])
        X_cell = pd.concat(
            [data_dict[mod].add_suffix(f'_{mod}') for mod in cell_data_types], axis=1
        )
        cell_sizes = [data_dict[mod].shape[1] for mod in cell_data_types]
    
        # Concatenate drug modalities
        drug_data_types = sorted([mod for mod in data_dict.keys() if mod.startswith("drug")])
        X_drug = pd.concat(
            [data_dict[mod].add_suffix(f'_{mod}') for mod in drug_data_types], axis=1
        )
        drug_sizes = [data_dict[mod].shape[1] for mod in drug_data_types]
    
        return X_cell, X_drug, cell_sizes, drug_sizes
    ...
    def load_data(data_modalities, raw_file_directory, screen_file_directory, sep, drug_directory=DRUG_DATA_FOLDER):
        ...
        if screen_file_directory is not None:
    ...
    def load_raw_files(raw_file_directory, data_modalities, intersect=True):
        ...
        for file in tqdm(files, 'Reading Raw Data Files...'):
            if any([file.startswith(x) for x in data_modalities]):
                ...
                # Convertir les colonnes en numériques, sinon mettre NaN
                ...
                df[df_normalized.columns] = df_normalized.fillna(0)
                if intersect:
